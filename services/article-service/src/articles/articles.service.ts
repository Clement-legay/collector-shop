...
export class ArticlesService {
    ...
    async findAll(filters?: { status?: ArticleStatus; sellerId?: string }): Promise<Article[]> {
        ...
        if (filters?.sellerId) {
            query.andWhere('article.sellerId = :sellerId', { sellerId: filters.sellerId });
        }

        // Exclude deleted articles by default
        query.andWhere('article.status != :deleted', { deleted: ArticleStatus.DELETED });
    ...
    async update(id: string, updateArticleDto: UpdateArticleDto): Promise<Article> {
        ...
        if (updateArticleDto.price !== undefined && updateArticleDto.price !== article.price) {
    ...
    async updatePrice(id: string, updatePriceDto: UpdatePriceDto): Promise<Article> {
        const article = await this.findOne(id);
